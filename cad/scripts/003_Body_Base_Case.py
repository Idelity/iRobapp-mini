...
# Description: iRobapp-mini用 メインボディ
...
# すべての窓穴を結合して一気に削る（スピーカー用のスリットは削除済）
cutter = cut_shapes
...
# ---------------------------------------------------------
# 4. 内側の固定ホルダー・リブの追加（左内壁接地リブだけを一体化）
# ---------------------------------------------------------
add_shapes = []

# --- (E) 内部：MAX98357Aアンプ用の左内壁接地リブ ---
rib_w = 6.0   
rib_t = 3.0   
rib_h = 15.0  
...
print("003_Body_Base_Case.py: スピーカーリングとスリットが削除され、アンプホルダーだけが綺麗に残りました！")
servo_pitch = 27.5     # ネジピッチ
...
# ---------------------------------------------------------
# 4. 内側の固定ホルダー・リブの追加（安全マージン最大確保仕様）
# ---------------------------------------------------------
add_shapes = []

# --- (F) 内部：12mmスピーカー固定用ホルダー ---
spk_center_x = 23.5
spk_center_y = -3.0
spk_cup_outer = Part.makeCylinder(spk_r + 1.5, spk_h, App.Vector(spk_center_x, spk_center_y, 15.0))
spk_cup_inner = Part.makeCylinder(spk_r, spk_h + 1.0, App.Vector(spk_center_x, spk_center_y, 15.0))
spk_holder = spk_cup_outer.cut(spk_cup_inner)
add_shapes.append(spk_holder)

# --- (G) 【最終微調整】内部：MAX98357Aアンプ用の左内壁接地リブ ---
# X座標を「-28.2」にほんの少しだけ内退させ、外壁の傾斜との干渉を100%完全にシャットアウト！
rib_w = 6.0   
rib_t = 3.0   
rib_h = 15.0  
...
print("003_Body_Base_Case.py: 1ピクセルの干渉も許さない、完璧な最終マスターデータがFIXしました！")
