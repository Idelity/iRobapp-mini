...
3. **【ボタンの位置についての注意】**
   * お使いのブラウザの画面幅やKaggleの仕様変更により、画面の最右上に「Download」ボタンが表示されない場合があります。
   * その場合は、**ページ中央のバリエーション選択（`gemma-2b-it-cpu-int4`）のすぐ右横にある「下矢印マーク（📥）」のアイコン**をクリックするか、ブラウザのウィンドウ横幅を最大まで広げて表示させてください。
   * ダウンロードされたファイル名が `gemma-2b-it-cpu-int4.bin` であることを確認してください。
...
└── 📁 iOSApp/
    └── 📁 iRobappMiniController/ (外箱フォルダ)
        ...
        └── 📁 iRobappMiniController/ (内側：コードフォルダ)
            ...
            ├── 📄 ContentView.swift
            └── 📄 gemma-2b-it-cpu-int4.bin  <-- ★ここへ直接保存する！
```
