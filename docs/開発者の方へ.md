...
6. ポップアップで選択ウィンドウが表示されるので、以下の通りに選択してファイルを確定させます。
   * **Framework（または形式）**: `TensorFlow 2` を選択
   * **Variation（バリエーション）**: `gemma-2b-it-cpu-int4` を選択
7. **【初回のみ：Google公式ライセンス同意画面が出ます】**
...
8. 同意完了後、自動的に `gemma-2b-it-cpu-int4.bin`（約1.35GB）のダウンロードが開始されます。
